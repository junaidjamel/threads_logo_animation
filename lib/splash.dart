...
class _SplashViewState extends State<SplashView>
  ...
  void initState() {
    ...
    // Phase 1 (0→40%):   draw on  0→1
    // Phase 2 (40→65%):  hold fully drawn
    // Phase 3 (65→95%):  undraw   0→1  (leading edge chases tail)
    // Phase 4 (95→100%): fade overlay out
        ...
        AnimatedBuilder(
          ...
          builder: (_, __) => Opacity(
            ...
            child: ColoredBox(
              ...
              child: Center(
                child: CustomPaint(
                  // Exact 166:192 aspect ratio from the SVG viewBox
...
/// Stroke-based draw/undraw animator for the Threads logo.
///
/// Paths are converted 1:1 from the official SVG (viewBox 0 0 166 192).
/// The SVG uses fill="none" stroke="white", so it is a stroked outline path.
///
/// The compound path has two subpaths:
///   [_pathOuter] — the full outer "@" outline, traced from top-center
///                  clockwise around the outside, inward through the spiral,
///                  around the "a" bowl interior, and back to start.
///   [_pathInner] — the closed teardrop counter inside the "a".
///
/// DRAW  : globalEnd advances 0→total  (pen draws forward along both paths)
/// UNDRAW: globalStart advances 0→total (tail retracts, erasing from beginning)
class _ThreadsPainter extends CustomPainter {
  ...
  // Outer path — every SVG command converted 1:1, in order.
  // SVG source: M84.0664 0.5 L85.375 ... closes back to H84.0664
  ...
  // Inner path — the "a" teardrop counter, closed shape.
  // SVG source: M91.8711 97.9678 ... Z
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Stroke width: the SVG has stroke-width=1 (default) in a 166x192 viewBox.
    // We draw with a slightly thicker stroke so it's visible at display size.
    // Adjust this value to taste — 1.5 matches the original logo weight well.
    final paint = Paint()
      ..color = Colors.white
